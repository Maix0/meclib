...
t_hashmap_C__PREFIX__ *new_hashmap_with_buckets_C__PREFIX__(
	...
	t_drop_C__PREFIX___fn drop, size_t buckets)
{
	...
	if (hmap == NULL)
		return (NULL);
	hmap->buckets = me_mem_alloc_array(buckets, sizeof(t_entry_C__PREFIX__ *));
								...
								C__VALTYPE__ value)
{
	...
	t_entry_C__PREFIX__ *prev = NULL;
	t_entry_C__PREFIX__ *entry = hmap->buckets[hashed_key % hmap->num_buckets];
	...
	if (entry == NULL)
	{
		entry = malloc(sizeof(t_entry_C__PREFIX__));
